...
def process_att_prep_press(date_from, date_to, pay_scheme_id, dept_code=None, company_id=None):
    conn = None
    ...
    try:
                ...
                (
                    ...
                    (
                        ...
                        WHERE da.attendance_date BETWEEN %s AND %s
                          AND da.worked_department_id = 9
                          AND da.worked_designation_id IN (114, 98)
                          AND wm.cata_id IN (3,4,5,6,7,9)
                          AND da.is_active = 1
                          AND da.company_id = %s
                          AND om.dept_id = 8
                          AND om.occu_code IN ('01', '02')
                    ) g
                    GROUP BY
                        attendance_date,
                        spell,
        ...
        return {
            "success": True,
            "message": "Press attendance preparation completed",
...
if __name__ == "__main__":
    try:
        if len(sys.argv) < 5:
            print(json.dumps({
                "success": False,
                "message": "Usage: python process_att_prep_press.py date_from date_to pay_scheme_id company_id [dept_code]"
            }))
            sys.exit(1)
        ...
        result = process_att_prep_press(date_from, date_to, pay_scheme_id, dept_code, company_id)
