...
def process_att_prep_wvg(date_from, date_to, pay_scheme_id, dept_code=None, company_id=None):
    conn = None
    ...
    try:
                ...
                (
                    ...
                    (
                        SELECT
                            ...
                            END AS nwhrs,
                            CASE
                                WHEN LENGTH(mech_code) > 4 THEN SUBSTR(mech_code, 4, 3)
                                ELSE mech_code
                            END AS mech_code
                        ...
                        WHERE da.attendance_date BETWEEN %s AND %s
                          ...
                          AND da.company_id = %s
                          AND om.dept_id = 7
                          AND om.occu_code IN ('02','03','05','06')
                    ) g
                    GROUP BY
                        attendance_date,
                        spell,
        ...
        return {
            "success": True,
            "message": "Weaving attendance preparation completed",
...
if __name__ == "__main__":
    try:
        if len(sys.argv) < 5:
            print(json.dumps({
                "success": False,
                "message": "Usage: python process_att_prep_wvg.py date_from date_to pay_scheme_id company_id [dept_code]"
            }))
            sys.exit(1)
        ...
        result = process_att_prep_wvg(date_from, date_to, pay_scheme_id, dept_code, company_id)
