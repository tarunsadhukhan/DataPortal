...
//		 $this->varaha->print_arrays($list);
		...
		foreach ($list as $loc) {
			...
			$row[] = $loc->Department;
			$row[] = $loc->Item_Code;
			$row[] = $loc->Item_Description;
			$row[] = $loc->Unit;

		//	$close_bales = ($loc->Opening_Bales + $loc->Receipt_Bales) - ($loc->Sold_Bales + $loc->Issue_Bales);
			$row[] = $loc->Cost_Center;

			$row[] = $loc->Issue_Quantity;
			$row[] = $loc->Issue_Value;
			$row[] = $loc->EXP_Type;
			$row[] = $loc->Branch;
 
			$row[] = $loc->SR_No;			
			$row[] = $loc->Mechine_Name;
	...
	public function ajax_list_inventory_minmax(){
		...
		$to_date=$_POST['to_date'];
	//	echo 'sub menu ajax'.$submenuId;
		$list = $this->Store_Inventory_Min_Max_Report_model->get_datatables($mainmenuId,$submenuId, $companyId, $from_date,$to_date);
//		 $this->varaha->print_arrays($list);
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $loc) {
			...
			$row[] = $loc->Min_Qty;
			$row[] = $loc->Max_qty;
			$row[] = $loc->Min_Order_qty;

		//	$close_bales = ($loc->Opening_Bales + $loc->Receipt_Bales) - ($loc->Sold_Bales + $loc->Issue_Bales);
			$row[] = $loc->Stock_qty;

			$row[] = $loc->Pending_indent_qty;
			$row[] = $loc->Pending_PO_Qty;
			$row[] = $loc->record_type;
			
			$row[] = $loc->Qty_To_Be_Order;
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->Store_Inventory_Min_Max_Report_model->count_all($mainmenuId,$submenuId, $companyId, $from_date,$to_date),
						"recordsFiltered" => $this->Store_Inventory_Min_Max_Report_model->count_filtered($mainmenuId,$submenuId, $companyId, $from_date,$to_date),
						"data" => $data,
				);
		//output to json format
	//	 $this->varaha->print_arrays($output);
	...
	public function ajax_list_inventory_allitems(){
...
//		 $this->varaha->print_arrays($list);
		...
		$no = $_POST['start'];
 		foreach ($list as $loc) {
			...
			$row[] = $loc->open_qty;
			$row[] = $loc->open_val;
			$row[] = $loc->tranrecv_qty;
			$row[] = $loc->tranrecv_val;
			$row[] = $loc->tranissu_qty;
			$row[] = $loc->tranissu_val;
			$row[] = $loc->clos_qty;
			$row[] = $loc->clos_val;
			$row[] = $loc->lrecpdate;
			$row[] = $loc->lissuedate;
	...
	public function ajax_list_mr_wise(){
		...
		$list = $this->mr_wise_model->get_datatables($mainmenuId,$submenuId, $companyId, $from_date,$to_date);
		// $this->varaha->print_arrays($list);
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $loc) {
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->mr_wise_model->count_all($mainmenuId,$submenuId, $companyId, $from_date,$to_date),
						"recordsFiltered" => $this->mr_wise_model->count_filtered($mainmenuId,$submenuId, $companyId, $from_date,$to_date),
						"data" => $data,
				);
		//output to json format
		// $this->varaha->print_arrays($output);
	...
	//		echo 'dashboard stores';
		...
		$this->data['from_date'] = date('Y-m-01',time());
			...
			$this->page_construct('store/storedashboard',$this->data);
		}
	}
...
//					print_r($submenuId);
					if($submenuId==185){
							$this->data['function'] = "ajax_list_stores_issue_checklist";
								...
								$this->data['function'] = "ajax_list_inventory_minmax";
						}else if($submenuId==487){
							$this->data['function'] = "ajax_list_inventory_allitems";
					}
						else if($submenuId==530){
							$this->data['from_date'] = date('Y-m-d',time());
							$this->data['function'] = "ajax_list_jute_mr_in_stock";
						}else if($submenuId==1855){
			...
			//			$this->data['godowns'] = $this->varaha_model->getAllGodownsNos();			
				...
				//		echo 'get cols';						
			//			var_dump($this->data['menuName']);
			//			echo 'menuname';
			//			var_dump($this->data['menudit']);
			//			echo 'menunid';
		...
		if($this->session->userdata('userid')==''){
		 ...
		 $this->data['mrno'] = $_POST['mr_no'];
			...
			if($this->data['submenuId']==185){
				...
				$this->data['function'] = "ajax_list_inventory_minmax";				
			}else if($this->data['submenuId']==487){
				$this->data['function'] = "ajax_list_inventory_allitems";				
			}
				...
				echo 'ananaaaaaa';
					...
					$this->data['res'] = $this->Store_Inventory_Min_Max_Report_model->directReport($perms);
				...
				//	$this->data['res'] = $this->store_issue_list_report_model->directReport($perms);
					...
					$html = $this->load->view('store/Inventory_report_allitems', $this->data, true);
				}
				if($this->data['submenuId']==530){					
