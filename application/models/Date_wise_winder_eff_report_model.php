<?php
class Date_wise_winder_eff_report_model extends CI_Model
{
...
$sql="select dprd.*,pprd.* from (
    SELECT vps.eb_no ebno,vps.eb_id,CONCAT(trim(wm.worker_name), ' ', IFNULL(trim(wm.middle_name), ''), ' ', IFNULL(trim(wm.last_name), '')) AS wname,
           tran_date loom_date,
           SUM(prod) AS prod,
    ...
    WHERE vps.tran_date between '".$from_date."' and '".$to_date."' and vps.company_id =".$companyId."
    AND atthrs IS NOT NULL
    GROUP BY vps.eb_no,vps.eb_id, tran_date,CONCAT(trim(wm.worker_name), ' ', IFNULL(trim(wm.middle_name), ''), ' ', IFNULL(trim(wm.last_name), '')) 
    ) dprd left join 
