<?php
class Store_issue_is03_report_model extends CI_Model
{
...
//		echo $itemdesc;
						...
						$sql="select k.*,round((Production+OVERHAULING+MAINTENANCE+CAPITAL+GENERAL),2) total_amt from (
							select company_id,branch_id,order_id,dept_desc,
							...
							from (
							select company_id,branch_id,dept_id,dept_desc,order_id,indent_type_desc,
							round(sum(iss_val),2) issue_value from  (
							select sih.company_id,sih.branch_id,sih.indent_type_id ,sitm.indent_type_desc, 
							sih.dept_id,md.dept_desc ,md.order_id , issue_qty,issue_qty * tpid.rate iss_val,issue_value issval,tpid.rate 
							from scm_issue_hdr sih
							left join master_department md on md.rec_id =sih.dept_id
							...
							) g group by company_id,branch_id,dept_id,dept_desc,indent_type_desc,order_id
							) m group by company_id,branch_id,dept_id,dept_desc,order_id
		...
		$sql .= "	order by order_id,dept_desc
