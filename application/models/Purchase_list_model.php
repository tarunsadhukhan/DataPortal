...
class Purchase_list_model extends CI_Model
{
	...
	private function _get_datatables_query($mainmenuId,$submenuId, $companyId, $from_date,$to_date)
	{
		...
		$this->db->where("po.po_approve_date>= '".date('Y-m-d',strtotime($from_date))." 00:00:00' and po.po_approve_date<= '".date('Y-m-d',strtotime($to_date))." 00:00:00'");
		if($companyId==118){
	...
	public function directReport($pers){
		...
		$this->db->join('status_master sm2' , 'po.status =sm2.status_id', 'left');
		$this->db->join('customer_master cm2' , 'po.customer =cm2.id', 'left');
		$this->db->where("po.po_approve_date>= '".date('Y-m-d',strtotime($pers['from_date']))." 00:00:00' and po.po_approve_date<= '".date('Y-m-d',strtotime($pers['to_date']))." 00:00:00'");
