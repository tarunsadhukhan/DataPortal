<?php
class Store_issue_is01_report_model extends CI_Model
{
...
//		echo $itemdesc;
        ...
        $sql="select k.*,(Production+OVERHAULING+MAINTENANCE+CAPITAL+GENERAL) total_amt from (
			...
			from
				...
				select
					...
					itemcode,
					item_desc,
					round(sum(iss_val), 2) issue_value
				from
					(
					select
						sih.company_id,
						...
						i.group_code,
						i.item_code,
						concat(i.GROUP_CODE, i.item_code) itemcode,
						i.item_desc ,
						issue_qty,
				(case
        ...
        when (`tpid`.`discount_mode` = 2) then (`tpid`.`rate` - ((`tpid`.`rate` * `tpid`.`discount`) / 100))*sih.issue_qty 
					...
					from
						scm_issue_hdr sih
					left join itemmaster i on sih.item_id =i.item_id 
