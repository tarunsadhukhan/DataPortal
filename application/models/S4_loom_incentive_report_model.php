<?php
class S4_loom_incentive_report_model extends CI_Model
{
...
//echo date('Y-m-d', strtotime($Date. ' + 10 days'));
    ...
    $sql="select allprd.*,CONCAT(trim(wm.worker_name), ' ', IFNULL(trim(wm.middle_name), ''), ' ', IFNULL(trim(wm.last_name), '')) AS wname, ifnull(total_inc,0) total_inc from ( 
        select dprd.*,avgeff,pprd.no_of_days from (
        select compid,
        ebno,loom_date,sum(prod) prod,sum(whrs) whrs,round(sum(eff*whrs)/sum(whrs),0) eff
        from (
        ...
        (
        select compid,
        ebno,count(*) no_of_days,sum(whrs) whrs,round(sum(eff*whrs)/sum(whrs),0) avgeff from (
        select compid,
        ebno,loom_date,sum(prod) prod,sum(whrs) whrs,round(sum(eff*whrs)/sum(whrs),0) eff
        from (
        ...
        left join (select * from EMPMILL12.tbl_incentive_data tid where tid.department_id=21) tid on tid.eff_inc=allprd.eff and allprd.compid=tid.company_id
        join worker_master wm on allprd.ebno=wm.eb_no and allprd.compid=wm.company_id
  ";
...
$query=rtrim($query)."
	count(*) 'Total_Days',round(sum(whrs*eff)/sum(whrs),0)	'avgeff',sum(total_inc) total_inc_amt,
    COUNT(CASE WHEN total_inc > 0 THEN 1 END) AS inc_days
";
	
	$cmpn='Njm';

$query=rtrim($query, ", ");
$query=$query." from ( " .$sql .") h group by ebno,wname 
order by avgeff,ebno";
...
";
    ...
    $sql="select allprd.*,CONCAT(trim(wm.worker_name), ' ', IFNULL(trim(wm.middle_name), ''), ' ', IFNULL(trim(wm.last_name), '')) AS wname, ifnull(total_inc,0) total_inc from ( 
        select dprd.*,pprd.no_of_days,avgeff from (
        select compid,
        ebno,loom_date,sum(prod) prod,sum(whrs) whrs,round(sum(eff*whrs)/sum(whrs),0) eff
        from (
        select loom_date,company_id compid,loom_id,ticket_no_a1 ebno,'A1' spell,'A' Shift,quality_code_a1 qcode,production_a1 prod,working_hrs_a1 whrs,
        efficiency_a1 eff from cuts_jugar_buff_1 cjb  
        where production_a1>0 and loom_date between '".$pers['from_date']."' and '".$pers['to_date']."' and 
        swd.company_id=".$pers['company']."
        union all
        select loom_date,company_id compid,loom_id,ticket_no_a2 ebno,'A2' spell,'A' Shift,quality_code_a2 qcode,production_a2 prod,working_hrs_a2 whrs,efficiency_a2 eff from cuts_jugar_buff_1 cjb  
        where production_a2>0 and loom_date between '".$pers['from_date']."' and '".$pers['to_date']."' and 
        swd.company_id=".$pers['company']."
                union all
        select loom_date,company_id compid,loom_id,ticket_no_b1 ebno,'B1' spell,'B' Shift,quality_code_b1 qcode,production_b1 prod,working_hrs_b1 whrs,efficiency_b1 eff from cuts_jugar_buff_1 cjb  
        where production_b1>0 and loom_date between '".$pers['from_date']."' and '".$pers['to_date']."' and 
        swd.company_id=".$pers['company']."
                union all
        select loom_date,company_id compid,loom_id,ticket_no_b2 ebno,'B2' spell,'B' Shift,quality_code_b2 qcode,production_b2 prod,working_hrs_b2 whrs,efficiency_b2 eff from cuts_jugar_buff_1 cjb  
        where production_b2>0 and loom_date between '".$pers['from_date']."' and '".$pers['to_date']."' and 
        swd.company_id=".$pers['company']."
                union all
        select loom_date,company_id compid,loom_id,ticket_no_c ebno,'C' spell,'C' Shift,quality_code_c qcode,production_c prod,working_hrs_c whrs,efficiency_c eff from cuts_jugar_buff_1 cjb  
        where production_c>0 and loom_date between '".$pers['from_date']."' and '".$pers['to_date']."' and 
        swd.company_id=".$pers['company']."
               ) dprd left join EMPMILL12.tbl_loom_master_other_details tlmod on dprd.loom_id=tlmod.mechine_id
        where tlmod.loom_type_id=1 
        group by compid,
...
$sql="select allprd.*,CONCAT(trim(wm.worker_name), ' ', IFNULL(trim(wm.middle_name), ''), ' ', IFNULL(trim(wm.last_name), '')) AS wname, ifnull(total_inc,0) total_inc from ( 
    ...
    union all
    select loom_date,company_id compid,loom_id,ticket_no_c ebno,'C' spell,'C' Shift,quality_code_c qcode,production_c prod,working_hrs_c whrs,efficiency_c eff from cuts_jugar_buff_1 cjb  
    where production_c>0 and loom_date between '".$pers['from_date']."' and '".$pers['to_date']."' and company_id =".$pers['company']."        ) dprd left join EMPMILL12.tbl_loom_master_other_details tlmod on dprd.loom_id=tlmod.mechine_id
    where tlmod.loom_type_id=1 
    group by compid,
    ebno,loom_date
    ) dprd
...
$query=rtrim($query, ", ");
$query=$query." from ( " .$sql .") h group by ebno,wname order by avgeff,ebno";
...
//echo $sql;
     ...
     for ($i = 2; $i < $q->num_fields() - 1; $i++) {
        $total_row[] = '';
    }
    $total_row[] = $tamt; // Add the total amount in the last column
    $data[] = $total_row;
          //   var_dump($data);
